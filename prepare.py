"""
One-time data preparation for autoresearch-mamba experiments (PyTorch/CUDA).
...
# ---------------------------------------------------------------------------
# Runtime utilities (imported by train_mamba.py)
# ---------------------------------------------------------------------------
...
def make_dataloader(tokenizer, B, T, split, buffer_size=1000):
    ...
    Yields (inputs, targets, epoch) as torch.Tensor (int64, CPU).
    """
    ...
    while True:
        ...
        inputs = torch.from_numpy(row_buffer[:, :-1].copy()).long()
        targets = torch.from_numpy(row_buffer[:, 1:].copy()).long()
...
def evaluate_bpb(model, tokenizer, batch_size):
    ...
    for _ in range(steps):
        ...
        log_probs = torch.log(torch.softmax(logits, dim=-1) + 1e-10)
        V = logits.shape[-1]
        y_flat = y.reshape(-1)
        log_probs_flat = log_probs.reshape(-1, V)
        # Gather log probs for target tokens
        loss_flat = -log_probs_flat[torch.arange(y_flat.shape[0], device=device), y_flat]
