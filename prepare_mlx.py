"""
One-time data preparation for autoresearch-mamba experiments (MLX).
...
Usage:
    ...
    AUTORESEARCH_MLX_PRESET_FILE=mlx_preset.local.json python prepare_mlx.py  # local reduced preset
...
PRESET = load_preset("prepare_mlx")

MAX_SEQ_LEN = int(PRESET.get("MAX_SEQ_LEN", 2048))
TIME_BUDGET = 300        # training time budget in seconds (5 minutes)
EVAL_TOKENS = int(PRESET.get("EVAL_TOKENS", 40 * 524288))
DEFAULT_NUM_SHARDS = int(PRESET.get("DEFAULT_NUM_SHARDS", 10))
...
def make_dataloader(tokenizer, B, T, split, buffer_size=1000):
    ...
    Yields (inputs, targets, epoch) as mx.array.
    """
...
def evaluate_bpb(model, tokenizer, batch_size):
    ...
    for _ in range(steps):
        ...
        loss_flat = -log_probs_flat[mx.arange(y_flat.shape[0]), y_flat]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepare data and tokenizer for autoresearch-mamba")
